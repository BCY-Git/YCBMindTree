...
export function Threads({
  ...
  useEffect(() => {
    ...
    const intersectionObserver = typeof IntersectionObserver === 'undefined'
      ? null
      : new IntersectionObserver((entries) => { isVisible = entries[0]?.isIntersecting ?? true }, { threshold: 0 })
    intersectionObserver?.observe(container)

    const prefersReducedMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ?? false
    const update = (time: number) => {
      if (!prefersReducedMotion) animationFrameId.current = requestAnimationFrame(update)
  ...
  return <div ref={containerRef} className={`threads-container ${className}`.trim()} aria-hidden="true" />
}
