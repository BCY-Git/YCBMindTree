...
export function WorkflowPanel({ session, suggestedGoal, busy, onStart, onChange, onCheckpoint, onDeposit, onGenerateAsset, onComplete }: {
  ...
  onCheckpoint: () => void
  onDeposit: () => void
  onGenerateAsset: (kind: WorkflowAssetKind) => void
  onComplete: () => void
}) {
  ...
  return <section className="workflow-panel">
    ...
    <div className="workflow-panel__actions"><button type="button" disabled={busy} onClick={onCheckpoint}>{busy ? '生成中…' : '生成检查点'}</button><button type="button" disabled={busy || !latest} onClick={onDeposit}>沉淀检查点</button>{session.mode === 'decide' && <button type="button" disabled={busy || !latest} onClick={() => onGenerateAsset('decision-record')}>生成决策记录</button>}{session.mode === 'explore' && <button type="button" disabled={busy || !latest} onClick={() => onGenerateAsset('knowledge-card')}>生成知识卡</button>}<button type="button" disabled={busy} onClick={onComplete}>完成协作</button></div>
  </section>
}
