...
import { isHtmlFile, normalizeHtmlFile } from '../attachments/html-attachment'
...
export function App() {
  ...
  const [leftPanelWidth, setLeftPanelWidth] = useState(() => loadPanelWidth('left'))
  ...
  const closeEditorPreferences = useCallback(() => setEditorPreferencesOpen(false), [])
  ...
  const uploadNodeAttachment = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    // HTML 文件统一归一化成 text/html，画布才会渲染预览卡片而不是普通附件条目。
    const file = isHtmlFile(raw) ? normalizeHtmlFile(raw) : raw
    try {
  ...
  return (
    <main className={`app-shell ${sidebarCollapsed ? 'is-sidebar-collapsed' : ''} ${inspectorCollapsed ? 'is-inspector-collapsed' : ''} ${assistantOpen ? 'is-assistant-open' : ''} ${mobilePanel === 'navigation' ? 'is-mobile-navigation-open' : ''} ${mobilePanel === 'inspector' ? 'is-mobile-inspector-open' : ''}`} style={{
      ...
      <section className="workspace">
        <aside className="left-rail">
          ...
          <footer className="sidebar-footer">
            <button className="sidebar-footer-action" type="button" onClick={() => setEditorPreferencesOpen(true)}><span><GearIcon /></span>设置</button>
        ...
        <section className="workspace-center">
          ...
          <div className="workspace-center__view">
            {workspaceView === 'map'
              ? <MindMapCanvas workspaceDocuments={taskDocuments} onRevealWorkspaceNode={revealWorkspaceNode} focusRootId={focusedNodeId} onChangeFocusRoot={setFocusedNodeId} editorPreferences={editorPreferences} />
        ...
        <aside className="inspector">
          <PanelResizeHandle side="right" width={rightPanelWidth} onChange={setRightPanelWidth} />
      ...
      <EditorPreferencesDialog layout={document.layout} onLayoutChange={(layout) => dispatch({ type: 'UPDATE_LAYOUT', layout })} open={editorPreferencesOpen} preferences={editorPreferences} onChange={updateEditorPreferences} onClose={closeEditorPreferences} />
