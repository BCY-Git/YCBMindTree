 ...
 * 2. 从中心根和各自由主题根出发 DFS 遍历，所有树上无循环（seen set 防重）
...
export function assertValidDocument(document: MindMapDocument): void {
  ...
  for (const topic of freeTopics) if (topic.parentId !== null) throw new Error('自由主题必须是独立根节点')
  ...
  const visit = (id: string) => {
    ...
    for (const childId of node.childIds) {
      ...
      if (child.isFreeTopic) throw new Error('自由主题必须是独立根节点')
      visit(childId)
    }
  }
  visit(document.rootId)
  freeTopics.forEach((topic) => visit(topic.id))
  // 确保每个节点都归属于中心主题或某一棵自由主题子树。
  ...
  for (const boundary of document.boundaries) {
    ...
    if (!parent || boundary.nodeIds.length < 2) throw new Error('边界父节点无效')
  ...
  for (const summary of document.summaries) {
    ...
    if (!parent || summary.nodeIds.length < 2) throw new Error('摘要父节点无效')
