...
export function createWorkflowSession(documentId: string, focusNodeId: string, goal: string, mode = inferWorkflowMode(goal)): WorkflowSession {
  const now = Date.now()
  return { id: crypto.randomUUID(), documentId, focusNodeId, mode, phase: 'context', status: 'active', goal: goal.trim(), audience: '', deliverable: '', constraints: [], acceptanceCriteria: [], confirmedFacts: [], rejectedOptions: [], openQuestions: [], nextActions: [], checkpoints: [], createdAt: now, updatedAt: now, completedAt: null }
}
