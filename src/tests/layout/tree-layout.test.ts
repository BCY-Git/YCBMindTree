...
describe('tree layout', () => {
  it('reserves intrinsic aspect-ratio space for an image pasted into a node', () => {
    const document = createInitialDocument()
    const nodeId = document.nodes[document.rootId].childIds[0]
    document.nodes[nodeId].attachments.push({
      id: crypto.randomUUID(), name: '图示.png', type: 'image/png', size: 1, createdAt: Date.now(),
      image: { width: 1600, height: 900, displayWidth: 320 },
    })

    const node = layoutTree(document).find((item) => item.id === nodeId)!

    expect(node.width).toBeGreaterThanOrEqual(340)
    expect(node.height).toBeGreaterThanOrEqual(232)
  })
  ...
  it('packs root leaf branches tightly around a single expanded branch', () => {
    ...
    expect(positions[1] - positions[0]).toBe(60) // 44px 卡片 + 默认 16px 同级间距
