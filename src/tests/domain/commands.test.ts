...
import { createNodeClipboard, executeCommand } from '@/domain/commands'
...
describe('MindTree command executor', () => {
  ...
  it('persists image attachment dimensions and rejects invalid resize data', () => {
    const document = createInitialDocument()
    const nodeId = document.nodes[document.rootId].childIds[0]
    const attachment = { id: crypto.randomUUID(), name: '结构图.png', type: 'image/png', size: 1024, createdAt: Date.now() }
    const attached = executeCommand(document, { type: 'ADD_NODE_ATTACHMENT', nodeId, attachment }).document
    const resized = executeCommand(attached, {
      type: 'SET_NODE_ATTACHMENT_IMAGE',
      nodeId,
      attachmentId: attachment.id,
      image: { width: 1600, height: 900, displayWidth: 320 },
    }).document

    expect(resized.nodes[nodeId].attachments[0].image).toEqual({ width: 1600, height: 900, displayWidth: 320 })
    expect(() => executeCommand(attached, {
      type: 'SET_NODE_ATTACHMENT_IMAGE',
      nodeId,
      attachmentId: attachment.id,
      image: { width: 1600, height: 900, displayWidth: 88 },
    })).toThrow('图片尺寸无效')
    expect(() => assertValidDocument(resized)).not.toThrow()
  })
